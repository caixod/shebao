 ...
 * */
class MobileDopayController extends BaseController {
    ...
    public function success_pay(){
        ...
        if($table_name && $request_num){
            ...
            if($result['data']['status']=='SUCCESS'){
                ...
                $_SESSION['order_data']='';
                $this->success("订单支付成功",U("Home/MobileOrder/index"));
            }else{
                $this->error("订单支付失败",U("Home/MobileOrder/index"));
            }
//            dump($result);
        }else{
            $this->error("订单不存在",U("Home/Mobile/shebao_dj"));
        }
...
//             $this->error("订单支付失败,请重新下单",U("Home/Order/index"));
        }
...
//              file_put_contents('/data/www/pc.zhongguoshebaodaili.cn/wwwroot/aaa.html',print_r($data,true),FILE_APPEND);
                  if ($req_bak['code'] == 200) {
                      ...
                      $this->error("支付请求失败,请重新下单", U("Home/MobileOrder/index"));
                  }
                  }else{
                  $this->success("订单已支付", U("Home/MobileOrder/index"));
                  }

              } else {
                  $this->error("支付参数错误,请重新下单", U("Home/Mobile/shebao_dj"));
              }

        }else{
            $this->error("错误,请重新下单",U("Home/Mobile/shebao_dj"));
        }
