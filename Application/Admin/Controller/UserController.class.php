...
//        dump($where);
        ...
        if($account_name){
            $Page->parameter['account_name']=$account_name;
        }
...
//        dump($order_mess);
    ...
    public function ajax_del(){
        $id=I('get.id');
        $table=I('get.table');
        $ret= D("{$table}")->where(['user_id'=>$id])->setField('is_lock',1);
        if($ret){
    ...
    public function edit_user(){
        ...
        if(IS_POST){
            ...
            if($model->create()){
                unset($model->user_id);
                $ret=$model->where(['user_id'=>$user_id])->save();
                if($ret){
        ...
        $userid=I("get.id");
        $info=$model->where(['user_id'=>$userid])->find();
