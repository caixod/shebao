...
class AdminController extends BaseController {
    ...
    public function admin_info(){
    	$admin_id = I('get.admin_id',0);//修改或者添加
		//取出所有的store
    	...
    	if($admin_id){
    		$info = D('admin')->where("admin_id=$admin_id")->find();
                $info['password'] =  "";
			  $cid_arr=array();
			if(strlen($info['cid'])>1){  //取出该用户所属的store_id
				$cid_arr=explode(',',$info['cid']);
			}else{
				$cid_arr[]=$info['cid'];
			}
			$info['cid']=$cid_arr;
...
//    	$this->assign('store_data',$store_data);
    ...
    public function adminHandle(){
		...
		$data['cid']=change_str($data['cid']);//数组转化为字符串
    ...
    public function role_info(){
		...
		$right = M('system_menu')->order('id')->where(['is_del'=>0])->select();
...
//		dump($modules);
		//权限组
		$group = array('system'=>'系统设置','member'=>'用户管理','wechat'=>'微信管理',
				'order'=>'订单中心','tools'=>'插件工具','manage'=>'基础设置','count'=>"统计报表"
		);
