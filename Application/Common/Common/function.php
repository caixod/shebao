 ...
 *
 * */
function getCityCodeFromLngLat($longitude,$latitude){
    $switch_ll = switchAnyLngLatToAmap($longitude,$latitude,'gps');
    $longitude = $switch_ll['lng'];
    $latitude = $switch_ll['lat'];
    $amap_url = C("AMAP_RESTAPI_URL")."key=".C("AMAP_KEY")."&location=".$longitude.",".$latitude;
    $res = file_get_contents($amap_url);
...
/*其它座标转换为高德座标
   float@longitude    经度
 * float@latitude   纬度
 * string@type  被转换座标类型 ,原坐标系，可选值：gps;mapbar;baidu;autonavi(不进行转换)
 * array@return   当前信息数组
*/

function switchAnyLngLatToAmap($lng,$lat,$type="gps"){
    $amap_url = C("AMAP_SWITCH_LNG_LAT_URL")."&key=".C("AMAP_KEY")."&output=JSON&coordsys=".$type."&locations=".$lng.",".$lat;
    $res = file_get_contents($amap_url);
    $res = json_decode($res,true);
    $lng_lat_arr = explode(',',$res['locations']);
    $res_arr = array(
        'lng' => $lng_lat_arr[0],
        'lat' => $lng_lat_arr[1]
    );
 ...
 * double@lng2  经度2
 * double@lat2  纬度2
 * string@type  返回类型 m米、km 千米
 * int@decimal  保留小数点后位数
 * float@return   当前信息数组
*/
function getDistanceBetweenCoordinate($lng1,$lat1,$lng2,$lat2,$type='km',$decimal=2){
    ...
    $a = $lng1-$lng2;
    $b = $lat1-$lat2;
    $s = 2 * asin(sqrt(pow(sin($b/2),2) + cos($lat1) * cos($lat2) * pow(sin($a/2),2)));
    $s = $s * EARTH_RADIUS;
    $s = round($s * 1000);
    if ($type == 'km')
    {
