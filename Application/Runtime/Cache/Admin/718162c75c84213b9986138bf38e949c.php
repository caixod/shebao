					...
					<div class="col-sm-12">
						<form action="" method="post">
							<table class="table table-bordered table-hover dataTable" id="example2" role="grid" aria-describedby="example2_info">
								<thead>
								<tr role="row">
									<th style="width: 25%" class="sorting" tabindex="0" aria-controls="example2" aria-label="Rendering engine: activate to sort column ascending">菜单名称</th>
									<th style="width: 25%" class="sorting_desc" tabindex="0" aria-controls="example2" aria-label="Browser: activate to sort column ascending" aria-sort="descending">类型</th>
									<th style="width: 25%" class="sorting" tabindex="0" aria-controls="example2" aria-label="Platform(s): activate to sort column ascending">类型值</th>
									<th style="width:25%" class="sorting conditional" tabindex="0" aria-controls="example2" aria-label="Platform(s): activate to sort column ascending">是否为个性化菜单&nbsp;&nbsp;<i class="fa fa-info-circle"></i></th>
								</tr>
								</thead>
								<tbody id="tbody">
...
//		alert(pmenu.length);
	...
	function addcmenu(pid){
		...
		i++;
		var id = i;
		var tpl = '<tr role="row" class="odd pmenu__pid__ menu__id__" ><td class="" style="padding-left: 5em"><input type="text" name="menu[__id__][name]" value="" placeholder="菜单名称"><a onclick="delmenu(__id__);" class="btn btn-danger"><i class="fa fa-trash-o"></i></a></td><td class="sorting_1"><select  name="menu[__id__][type]" style="width: 50%"><option value="view">链接</option><option value="click">触发关键字</option><option value="scancode_push">扫码</option><option value="scancode_waitmsg"> 扫码（等待信息）</option><option value="pic_sysphoto">系统拍照发图</option><option value="pic_photo_or_album">拍照或者相册发图</option><option value="pic_weixin">微信相册发图</option><option value="location_select">地理位置</option></select></td><td><input style="width: 100%" type="text" value="" name="menu[__id__][value]" placeholder="菜单值"></td><input style="width: 100%" name="menu[__id__][pid]" type="hidden" value="__pid__"><td><select name="menu[__id__][tag_id]" class="tag_menu_mark"><option value="0">否</option> <?php if(is_array($tags)): foreach($tags as $key=>$t): ?><option value="<?php echo ($t["tag_id"]); ?>"><?php echo ($t["tag_name"]); ?></option><?php endforeach; endif; ?></select></td></tr>';
		tpl = tpl.replace(/__id__/g,id);
	...
	//////////////////
	$(function(){
		$(".conditional").mouseover(function(){
			layer.tips('如果选定个性化菜单，则该菜单只对对应标签下的用户可见', '.fa-info-circle', {
				tips: [1, '#000000'] //还可配置颜色
			});
		});

		////////////
		$("body").delegate(".tag_menu_mark","change",function(){

			if(va_lue=='n'){
				va_lue = $(this).val();
			}
			if($(this).val()!=va_lue && $(this).val()!=0){
				alert("目前个性化菜单只支持发布一个相同组");
				$(this).val(0);
				return false;
			}
			/*$("table").find(".tag_menu_mark").each(function(index,v){
				$(this).find("option").each(function(){
					if($(this).val()!=0 && $(this).val()!=va_lue){
						alert("目前个性化菜单只支持发布一个相同组");
						return false;
						//alert($(this).val());
						//alert('.tag_menu_mark option[value="'+$(this).val()+'"]');
						//$('.tag_menu_mark option[value="'+$(this).val()+'"]').remove();
					}
				});
