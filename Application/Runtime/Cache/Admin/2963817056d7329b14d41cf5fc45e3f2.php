                                    ...
                                    <?php if(is_array($order_mess)): foreach($order_mess as $k=>$vo): ?><tr role="row" style="text-align: center;">
                                         ...
                                         <?php echo ($vo["status"]); ?>
                                              ...
                                              <td  style="width: 140px;">
                                                  <a href="<?php echo U('Admin/Order/yl_sgz_edit',array('id'=>$vo['id']));?>" data-toggle="tooltip" title="" class="btn btn-primary" data-original-title="处理订单"><i class="fa fa-pencil"></i></a>&nbsp;&nbsp;&nbsp;&nbsp;
    ...
    <script>
        ...
        function del(id)
        {
            if(!confirm('确定要删除吗?'))
                return false;
            $.ajax({
                url:"/index.php?m=Admin&c=Order&a=ajax_del&table=yl_sgz&id="+id,
                dataType:'json',
                type:'get',
                success: function(v){
