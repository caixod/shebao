...
{    
    ...
    /**
     * 服务器点对点响应操作给支付接口方调用
     *    home/payment/notify 调用了  bai
     */
    function response()
    {
        $order_sn = I("order_sn");
        if(strlen($order_sn)>28){
            require_once("example/dishNotify.php");
        }else{
    ...
    function getJSAPI($order){
        ...
        $input->SetNotify_url(SITE_URL.'/index.php/Home/Payment/notifyUrl/pay_code/weixin/order_sn/'.$order['order_sn']);
...
EOF;
    ...
    function getDishJSAPI($order){
        //if(stripos($order['order_sn'],'recharge') !== false){
            //$go_url = U('Mobile/User/points',array('type'=>'recharge'));
            //$back_url = U('Mobile/User/recharge',array('order_id'=>$order['order_id']));
        //}else{
            $go_url = U('Dish/Index/payCallBack',array('system_order_id'=>$order['system_order_id'],'go'=>'success','order_type'=>'waiting_take'));
            $back_url = U('Dish/Index/payCallBack',array('system_order_id'=>$order['system_order_id'],'go'=>'cancel','order_type'=>'waiting_pay'));
        //}
        //①、获取用户openid
        $tools = new JsApiPay();
        //$openId = $tools->GetOpenid();
        ...
        //②、统一下单
        $input = new WxPayUnifiedOrder();
        $input->SetBody("支付订单：".$order['system_order_id']);
        $input->SetAttach("weixin");
        //$input->SetOut_trade_no($order['system_order_id'].time());    //原系统定义
        $input->SetOut_trade_no($order['system_order_id']);
        $input->SetTotal_fee($order['actually_total_fee']);
        $input->SetTime_start(date("YmdHis"));
        $input->SetTime_expire(date("YmdHis", time() + 600));
        $input->SetGoods_tag("tp_wx_pay");
        $input->SetNotify_url(SITE_URL.'/index.php/Mobile/Payment/notifyUrl/pay_code/weixin/order_sn/'.$order['system_order_id']);
        $input->SetTrade_type("JSAPI");
        $input->SetOpenid($openId);
        $order2 = WxPayApi::unifiedOrder($input);
        //echo '<font color="#f00"><b>统一下单支付单信息</b></font><br/>';
